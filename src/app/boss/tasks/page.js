...
export default function TaskManager() {
  const { user, isAuthenticated, isAdmin, loading: authLoading, logout } = useUser();
  ...
  const loadAdminData = async () => {
    try {
      ...
      const [tasksData, staffData] = await Promise.all([
        ...
        apiCall('/api/staff/tasks/staff-list')
      ]);
  ...
  const loadStaffData = async () => {
    try {
      setLoading(true);
      const tasksData = await apiCall('/api/staff/tasks/staff');
      setTasks(tasksData.tasks || []);
    } catch (error) {
  ...
  const handleAssignTask = async (e) => {
    ...
    try {
      setLoading(true);
      const result = await apiCall('/api/staff/tasks/assign', {
        method: 'POST',
        body: JSON.stringify({
          ...assignForm,
          tags: assignForm.tags.split(',').map(tag => tag.trim()).filter(tag => tag)
        })
      });
  ...
  const handleUpdateProgress = async (taskId, progress) => {
    try {
      const result = await apiCall(`/api/staff/tasks/staff/${taskId}/progress`, {
        method: 'PUT',
  ...
  const handleUpdateStatus = async (taskId, status) => {
    try {
      const result = await apiCall(`/api/admin/tasks/${taskId}`, {
        method: 'PUT',
  ...
  const ProgressBar = ({ progress, size = 'md' }) => {
    ...
    return (
      <div className="w-full bg-gray-200 rounded-full">
        <div 
          className={`bg-gradient-to-r from-blue-500 to-blue-600 ${height} rounded-full transition-all duration-500 ease-out`}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-100 flex items-center justify-center p-4">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Please log in to access the task manager</h2>
          <button
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500 capitalize">{isAdmin ? 'admin' : 'staff'}</p>
              </div>
              <button
